...
export async function GET() {
    ...
    try {
        ...
        // --- NO SORTING LOGIC ---
        // Data will remain in the exact order it was read from the CSV file.
        // processedData.sort((a, b) => { ... }); // REMOVED

        // Add rank based on CSV row order (1st row = Rank 1, etc.)
        const rankedData = processedData.map((student, index) => ({
            ...student,
            rank: index + 1
        }));
        ...
        const responsePayload = {
            success: true,
            message: "Leaderboard data fetched successfully (Original CSV Order).",
            updatedAt: updatedAt,
            data: rankedData,
        };
